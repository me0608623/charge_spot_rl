...
import random
import shutil
from custom_envs.train_double_car.double_car import DoubleCar
from warp_drive.env_wrapper import EnvWrapper
from custom_envs.custom_trainer import Trainer
from matplotlib import animation
from custom_envs.train_double_car.generate_rollout_animation  import generate_tag_env_rollout_animation
...
class Spot_Env():
    def __init__(self):
        ...
        self.phase1_spot_batch = 47000
        self.phase1_goal_batch = 89000
        self.phase3_spot_batch = 50000
        self.phase3_goal_batch = 90000
    ...
    def Calculate_PHASE_Parameter(self, PHASE = 1, old_config = None):
        ...
        self.goal_random_map = True
        self.train_goal_fix_spot_model = True
        ...
        if PHASE == 1:
            ...
            self.phase_config["num_taggers"] = 6
            self.phase_config["num_walls"] = 7
            spot_entropy_coeff = 0.04
            self.train_goal_fix_spot_model = False
        elif PHASE == 2:
            self.train_spot_batch_size = self.phase3_spot_batch
            self.train_goal_batch_size = self.phase3_goal_batch
            self.train_goal_num_spot = 2
            self.phase_config["num_taggers"] = 6
            self.phase_config["num_walls"] = 7
        elif PHASE == 3:
            self.train_spot_batch_size = self.phase3_spot_batch
        ...
        if self.train_goal_num_spot is None: self.train_goal_num_spot = self.phase_config["num_taggers"]
        self.train_spot_num_spot =  self.phase_config['num_taggers']
        self.rl_fps = 3
        self.phase_config["episode_length"] = int(rl_env_times * self.rl_fps)
  
        one_sec_spot_gamma = 0.86
        one_sec_goal_gamma = 0.96
        spot_gamma = 1 - ((1 - one_sec_spot_gamma) / self.rl_fps)
        goal_gamma = 1 - ((1 - one_sec_goal_gamma) / self.rl_fps)
        ...
        self.wandb_config['goal_model'] = [512, 512, 512]  # layer big is useful but need to test other thing
        self.wandb_config['goal_policy_network'] = []
        self.wandb_config['goal_critic_network'] = []
        self.wandb_config['spot_random_agent'] = True
        self.wandb_config['spot_random_map'] = True
        self.wandb_config['goal_random_agent'] = True
        self.wandb_config['goal_random_map'] = self.goal_random_map
        ...
        self.wandb_config['spot_lr'] = 0.0005
        self.wandb_config['spot_entropy_coeff'] = spot_entropy_coeff
        self.wandb_config['spot_action2_entropy_coeff'] = spot_action2_entropy_coeff
        self.wandb_config['spot_action3_entropy_coeff'] = spot_action3_entropy_coeff
    ...
    def creat_RL_ENV(self):
        ...
        self.env_wrapper = EnvWrapper(
            DoubleCar(**run_config["env"]),
            num_envs = run_config["trainer"]["num_envs"],
            use_cuda = True,
        )
        policy_tag_to_agent_id_map = {
            "tagger": list(self.env_wrapper.env.taggers),
            "wall": list(self.env_wrapper.env.walls),
    ...
    def load_weight(self, wandb_path=None):
        if self.train_env_num == 1 and self.PHASE > 1 or wandb_path is not None:
            ...
            os.rename(spot_reg_file, self.env_dir_+ self.results_dir_ + "/tagger_" + self.spot_load_weight_name)
            shutil.copy2(spot_path, spot_reg_file)
            os.rename(spot_reg_file, self.env_dir_+ self.results_dir_ + "/tagger_1.state_dict")
            os.rename(goal_reg_file, self.env_dir_+ self.results_dir_ + "/goal_" + self.goal_load_weight_name)
            os.rename(obs_reg_file, self.env_dir_+ self.results_dir_ + "/wall_" + self.goal_load_weight_name)
            shutil.rmtree(artifact_dir)
        if self.train_env_num > 1 or self.PHASE > 1 or wandb_path is not None:
            self.spot_file_path = self.env_dir_+ self.results_dir_ + "/tagger_" + self.spot_load_weight_name
            self.goal_file_path = self.env_dir_+ self.results_dir_ + "/goal_" + self.goal_load_weight_name
            self.obs_file_path = self.env_dir_ + self.results_dir_ + "/wall_" + self.goal_load_weight_name
            self.new_spot_file_path = self.env_dir_+ self.results_dir_ + "/tagger_" + self.new_spot_load_weight_name
            self.new_goal_file_path = self.env_dir_+ self.results_dir_ + "/goal_" + self.new_goal_load_weight_name
            self.new_obs_file_path = self.env_dir_ + self.results_dir_ + "/wall_" + self.new_goal_load_weight_name
            self.trainer.load_model_checkpoint(
                {
                    "tagger": self.spot_file_path,
                    "goal": self.goal_file_path,
                    "wall": self.obs_file_path,
                }
            )
    ...
    def video_test(self, test_num_time =None):
        ...
        anim = generate_tag_env_rollout_animation(
            ...
            num_goals = self.phase_config["testing_num_goals"],
        )
    ...
    def remove_agents_old_weight(self):
        if self.train_env_num > 1:
            if self.train_goal:
                ...
                self.trainer.remove_old_weight({"wall": self.env_dir_+ self.results_dir_ + "/wall_" + self.goal_load_weight_name})
            if self.train_spot:
                self.trainer.remove_old_weight({"tagger": self.env_dir_+ self.results_dir_ + "/tagger_" + self.spot_load_weight_name})
    ...
    def log_all_data_to_wandb(self):
        ...
        model_path = "./custom_envs/models.py"
        ...
        python_code_path = "custom_envs/spot_3d/spot_3d.py"
        cuda_code_path = "custom_envs/spot_3d/spot_3d_step.cu"
        ...
        for PHASE in range(99):
            #try:
                ...
                for PHASE_num_env in range(phase_total_env):
                    ...
                    if self.train_spot:
                        self.phase_config['num_taggers'] = self.train_spot_num_spot 
                        self.phase_config['seed'] = int(10000 * random.random())
                    if self.train_goal:
                        self.phase_config['num_taggers'] = self.train_goal_num_spot
                        self.phase_config['seed'] = int(10000 * random.random())
                    else:
                        self.phase_config['num_taggers'] = self.train_spot_num_spot
                        self.phase_config['seed'] = int(10000 * random.random())
