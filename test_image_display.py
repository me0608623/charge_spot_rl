...
from custom_envs.spot_3d.generate_rollout_image import generate_tag_env_rollout_image
...
class Spot_Env():
    def __init__(self):
        ...
        self.phase1_spot_batch = 7000
        self.phase1_goal_batch = 2000
        self.action_x_data = []
        self.action_y_data = []
    ...
    def video_test(self, test_num_time =None):
        ...
        anim = generate_tag_env_rollout_image(
            self.trainer,
            use_wandb = True,
            PHASE_ = self.PHASE,
            state = test_num_time,
    ...
    def display_spot_custom_model_action(self):
        # if it not work cheak model.py data_dict["input"][16] idx shoudt be in game state
    ...
    def replace_old_file(self, wandb_path):
        import shutil
        self.real_env_dir = "spot_3dmodule"
        self.real_cuda_file = "/spot_3dmodule_step.cu"
        self.env_dir = "spot_3d"
        self.reg_dir = "old_env"
 
        #python code can alway spot_3d.py
        now_python_path =  "./custom_envs/" + self.env_dir + "/spot_3d.py"
        reg_python_path = "./custom_envs/" + self.reg_dir + "/spot_3d.py"
        ...
        #cuda code should be origin cu
        """cuda file use origin path"""
        now_python_path =  "./custom_envs/" + self.real_env_dir + self.real_cuda_file
        reg_python_path = "./custom_envs/" + self.reg_dir + self.real_cuda_file
        python_code_path = self.search_files_with_keywords(wandb_path, keywords = ['cuda_code'])
        shutil.copy2(now_python_path, reg_python_path)
        shutil.copy2(python_code_path, now_python_path)

        now_model_path =  "./custom_envs/test_model.py"
    ...
    def test(self,time_id, artifact_version, Start_Testing_type = 1, End_test_type = 2 ,test_num_time = 5, model_connect = None):
        self.Training = False
        ...
        for Testing_type_ in range(5):
            ...
            wandb_name = 'Evaluation test ' + str(Testing_type) + " " + artifact_version+ "_2"
            ...
            self.phase_config["goal_speed_rate_with_spot"] = 0.0
            self.phase_config["obstacle_speed_rate_with_spot"] = 0.7
            ...
            #run_time = [386, 428, 587, 312, 558, 162, 244, 596, 645, 800, 461, 15, 292, 570, 831, 401, 112]
            run_time = [461, 15, 292, 570, 831, 401, 112]  # stair test
            #run_time = [570] # 570, 
            if test_num_time > len(run_time): test_num_time = len(run_time)
            for time in range(test_num_time):
                if 2 + time + avoid_idx in avoid_seed:
                    ...
                    if 2 + time + avoid_idx in avoid_seed:
                        ...
                        if 2 + time + avoid_idx in avoid_seed:
                            avoid_idx += 1
                #self.phase_config['seed'] = time + 843
                self.phase_config['seed'] = run_time[time]
