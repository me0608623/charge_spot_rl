...
import random
import shutil
from custom_envs.merchandise.merchandise import Merchandise_Env
...
class RL_config():
    def __init__(self):
        env_config = dict(
            ...
            spot_state_obs_agent_rate = 0.5,  # if it need shoud be large like 0.8
            num_spot = 6,
            num_goals = 20,
            num_obstacle = 7,
            num_stairs = 3,
            rl_fps = 3,
            goal_size = 0.3,
            spot_reward_on_floor = 30.0,
            spot_penalty_hit = -5.0,
            spot_cost_operate = 0.03
        )
        train_config = dict(
            'entropy_coeff' = 0
...
class Multi_agent_config():
    def __init__(self):

        _, env_config = RL_config()

        one_sec_spot_gamma = 0.94
        one_sec_goal_gamma = 0.94
        spot_gamma = 1 - ((1 - one_sec_spot_gamma) / env_config["rl_fps"])
        goal_gamma = 1 - ((1 - one_sec_goal_gamma) / env_config["rl_fps"])
        ...
        train_config = dict(
            spot = dict(
                ...
                lr = 0.0005,
                entropy_coeff = 0.04,
                action2_entropy_coeff = 0.15,
                action3_entropy_coeff = 0.15,
                clip_grad_norm = True,
                max_grad_norm = 1.0,
                random_agent = True,
                random_map = True,
...
class Multi_phase_config():
    def __init__(self, use_Multi_agent = False, phase = 1):
        if use_Multi_agent: self.phase1_train_config, self.phase1_env_config = Multi_agent_config()
        else: self.phase1_train_config, self.phase1_env_config = RL_config()

        if phase > 1: train_config, env_config = self.calculate_phase_parameter(phase)
        else: 
            train_config = self.phase1_train_config
            env_config = self.phase1_env_config
        return train_config, env_config

    def calculate_phase_parameter(self, phase):
        train_config = self.phase1_train_config
        env_config = self.phase1_env_config
        if phase == 2:
            train_config['spot']['batch_size'] = 100000
            train_config['goal']['batch_size'] = 35000
        elif phase == 3:
            train_config['spot']['batch_size'] = 130000
            train_config['goal']['batch_size'] = 110000
...
class Multi_RL_Env():
    def __init__(self):
        self.Training = True
        self.START_PHASE = 1
        self.RL_env_name = "Merchandise_RL"
        self.env_dir_ = "./custom_model/Merchandise_RL/example/"
        self.results_dir_ = "test_train"
        self.wandb_columns = ["state", "video", "weight", "PreTrain", "PHASE", "Get Goals Probability", "Total Goal", "Hit the obstacle times"]
        self.notes = None #''
        self.rl_env_versioin = 1
        self.wandb_table_history = []
        self.error_msg_list = []
        self.train_num_env = 30  # 300
        self.wandb_phase_output_video_num = 2 # 10
        self.env_train_times = 20  # 101
        self.model_params_save_freq = 100

        use_Multi_agent = True
        use_Multi_phase = True
        if use_Multi_phase: self.train_config, self.env_config = Multi_phase_config(use_Multi_agent = use_Multi_agent)
        elif use_Multi_agent: self.train_config, self.env_config = Multi_agent_config()
        else: self.train_config, self.env_config = RL_config()
    ...
    def Calculate_PHASE_Parameter(self, PHASE = 1, old_config = None):
        ...
        if PHASE == 1:
            ...
            self.phase_config["num_stairs"] = 3
            ...
            self.train_goal_num_spot = 1
            self.phase_config["num_spot"] = 3  # 降低數量為了讓 spot 爬樓梯有價值
            ...
            self.phase_config["spot_penalty_hit"] = -5.0  # 為了上樓梯降低沒用，動態障礙物避障效果會很差，一下就死了
            self.phase_config["goals_reward_step"] = 0.8
            self.phase_config["goals_reward_speed"] = 0.5
            ...
            self.phase_config["num_goals"] = 3
            self.phase_config["num_obstacle"] = 2
            self.phase_config["num_stairs"] = 2
            self.phase_config["rl_fps"] = 3
            self.phase_config["goal_size"] = 0.29
            self.phase_config["spot_reward_on_floor"] = 3.0
            ...
            self.phase_config["obstacle_speed_rate_with_spot"] = 0.85
            self.phase_config["floor_width_bias"] = 6.5
        elif PHASE == 4:
            self.train_spot_batch_size = self.phase5_spot_batch
            self.train_goal_batch_size = self.phase5_goal_batch
            self.train_goal_num_spot = 1
            self.train_goal_num_goal = 5
            goal_entropy_coeff = 0.06
            goal_action3_entropy_coeff = 0.12
            rl_env_times = 50
            self.spot_times_train_env = False
            self.phase_config['Testing_type'] = 4
            self.phase_config["num_spot"] = 1
            self.phase_config["num_goals"] = 1
            self.phase_config["num_obstacle"] = 10
            self.phase_config["num_stairs"] = 1
            self.phase_config["num_wall"] = 3
            ...
            self.phase_config["obstacle_speed_rate_with_spot"] = 1.05
            self.phase_config["floor_width_bias"] = 6.5
            ...
            self.phase_config["num_stairs"] = 2
            self.phase_config["num_wall"] = 3
            self.phase_config["wall_length"] = 4.0
            self.phase_config["rl_fps"] = 3
            self.phase_config["goal_size"] = 0.29
            self.phase_config["spot_reward_on_floor"] = 0.0
            self.phase_config["spot_penalty_hit"] = -15.0
            ...
            # 目前看不出優點
            self.train_spot_batch_size = self.phase7_spot_batch
            ...
            # 優化避障
            self.train_spot_batch_size = self.phase8_spot_batch
            ...
            self.phase_config["rl_fps"] = 3
            self.phase_config["goal_size"] = 0.28
            self.phase_config["spot_reward_on_floor"] = 0.0
            self.phase_config["spot_penalty_hit"] = -30.0
            self.phase_config["goals_reward_step"] = 0.3
            self.phase_config["goals_reward_speed"] = 0.2
            self.phase_config["obs_size_rand"] = 0.3
            ...
            policy=dict(
                ...
                spot = dict(
                    ...
                    model = dict(
                        ...
                        memory_dim = self.wandb_config['memory_dim'],
    ...
    def log_all_data_to_wandb(self):
        ...
        model_path = "./custom_envs/fully_connected.py"
        ...
        python_code_path = "custom_envs/spot_3dIn/spot_3dIn.py"
        cuda_code_path = "custom_envs/spot_3dIn/spot_3dIn_step.cu"
