...
class Map():
    def __init__(
        ...
        if seed is not None: self.random_map_seed = seed + 17
        if random_map == False:  self.random_map_seed = seed + 23
        else: self.random_map_seed = int(random.random() * 10000)
        np.random.seed(self.random_map_seed + 1)

        self.num_wall = num_wall
        self.wall_length = wall_length
        self.grid_length = grid_length
        self.num_stairs = int(min(np.random.rand() * num_stairs + 1, num_stairs))
    ...
    def generate_map(self):
        ...
        np.random.seed(self.random_map_seed)
        wall_list = [
        ...
        for stair_idx in  range(self.num_stairs):
            ...
            origin_x =  (np.random.rand() - 0.5) * self.grid_length * 0.75 + grid_length_half
            origin_y =  (np.random.rand() - 0.5) * self.grid_length * 0.75 + grid_length_half
            ...
            self.stair_position.append(origin_x)  # down_x
            self.stair_position.append(origin_y)  # down_y
            self.stair_position.append(origin_x + stairs_length * up_or_down_y * (1 - right_or_left))   # up_x
            self.stair_position.append(origin_y + stairs_length * up_or_down_y * right_or_left)  # up_y
        ...
        for i in range(self.map_max_y):
            for j in range(self.map_max_x):
                ...
                if i == 0 or i == self.map_max_y - 1 or j == 0 or j == self.map_max_x - 1:  
                    self.map_z[i,j] = self.grid_length_height * 2
                    on_stair = True
                else:  self.map_z[i,j] = z_
                if on_wall and z_ == 0:  self.map_z[i,j] = self.grid_length_height * 2
                if on_stair or (on_wall and z_ == 0):  self.map_limit[i, j] = 1
...
class Spot_3d(CUDAEnvironmentContext, Map):
    ...
    def __init__(
        ...
        random_agent = False,
        random_map = False,
        spot_cooperate = False,
        need_reverse_env = False,
        num_obs_see = 10,
        spot_lidar_angle = 4,
        ...
        spot_state_obs_agent_rate = 0,
        preprocess_info_weight = [],
        car_mode = False
    ):
        """
        ...
        if car_mode:  self.car_mode = self.float_dtype(1)
        else:  self.car_mode = self.float_dtype(0)
        ...
        if seed is not None: self.np_random.seed(seed)
        if self.random_agent: self.np_random.seed(int(random.random() * 10000))
        if spot_cooperate and num_goals == 1:  self.spot_cooperate = self.float_dtype(1)
        else:  self.spot_cooperate = self.float_dtype(0)
        if need_reverse_env:  self.need_reverse_env = self.float_dtype(random.random() * 2) #  <1 mean display env will reverse env_z
        else:  self.need_reverse_env = self.float_dtype(0)
        ...
        for agent_id in range(self.num_agents):
            if agent_id in set(no_spot_):
                if agent_id in set(obstacle_):
                    if self.np_random.rand() < spot_state_obs_agent_rate: self.agent_type[agent_id] = 2  # agent state obstacles
                    else: self.agent_type[agent_id] = 3  # static obstacles
        ...
        self.network_feture = {
            agent_id: preprocess_info_weight
            for agent_id in range(self.num_agents)
        }
        self.num_preprocess_feture = self.float_dtype(len(preprocess_info_weight))
        ...
        self.stair_position = self.float_dtype(self.stair_position)
        ...
        for idx, spots_id in enumerate(spots_list):
            spot_hit_ = self.cheak_spot_hit(
                idx, 
                spots_id,
                agent_x = self.starting_location_x[spots_id],
                agent_y = self.starting_location_y[spots_id], 
                agent_dir = self.starting_directions[spots_id]
            )
            while(spot_hit_):
    ...
    def cheak_spot_hit(self, idx, spots_id, agent_x, agent_y, agent_dir):
        dir_dx = np.cos(agent_dir) 
        dir_dy = np.sin(agent_dir)
        # body wall
        compute_hit_wall_ = self.compute_hit_wall(
            agent_x,
            agent_y,
            self.agent_size[spots_id] + 0.6
        )
        if compute_hit_wall_ == True: return True
        # head wall
        point_x = agent_x + dir_dx * self.spot_head_distance
        point_y = agent_y + dir_dy * self.spot_head_distance
        compute_hit_wall_ = self.compute_hit_wall(
            point_x,
            point_y,
            self.agent_size[spots_id] + 0.6
        ...
        # ass wall
        point_x = agent_x - dir_dx * self.spot_ass_distance
        point_y = agent_y - dir_dy * self.spot_ass_distance
        compute_hit_wall_ = self.compute_hit_wall(
            point_x,
            point_y,
            self.agent_size[spots_id] + 0.6
        ...
        # body pitch
        agent_grid_x = min(int(agent_x / self.grid_density + 0.5), self.map_max_x - 1)
        agent_grid_y = min(int(agent_y / self.grid_density + 0.5), self.map_max_x - 1)
    ...
    def k_nearest_neighbors_obs(self, agent_id, k):
        ...
        for ag_id in range(self.num_agents):
            if (ag_id != agent_id) and (self.still_in_the_game[ag_id]) and (self.agent_type[ag_id] == 2 or self.agent_type[ag_id] == 3):
                agent_ids_and_distances += [
                    (ag_id, self.compute_distance(agent_id, ag_id))
                ]
        k_nearest_neighbor_ids_and_distances = heapq.nsmallest(
    ...
    def generate_observation(self):
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.still_in_the_game[agent_id]:
                ...
                obs_sec_speed = normalized_global_obs[2, nearest_neighbor_ids] * self.spot_max_speed_x
                obs_dir = normalized_global_obs[4, nearest_neighbor_ids]
                next_obs_x = normalized_global_obs[0, nearest_neighbor_ids] + obs_sec_speed * np.cos(obs_dir)
                next_obs_y = normalized_global_obs[1, nearest_neighbor_ids] + obs_sec_speed * np.sin(obs_dir)
                next_diff_x = next_obs_x - normalized_global_obs[0, agent_id]
                next_diff_y = next_obs_y - normalized_global_obs[1, agent_id]
                agent_view_global_obs_[2,nearest_neighbor_ids] = next_diff_x * np.cos(dir_agent) + next_diff_y * np.sin(dir_agent)
                agent_view_global_obs_[3,nearest_neighbor_ids] = -1 * next_diff_x * np.sin(dir_agent) + next_diff_y * np.cos(dir_agent)
                agent_view_global_obs_[4,nearest_neighbor_ids] = normalized_global_obs[0, nearest_neighbor_ids] * 0.0 
                ...
                obs_11 = np.array([front_agent_z - agent_z])
                obs_12 = np.array([back_agent_z - agent_z])
                obs_13 = np.array([left_agent_z - agent_z])
                obs_14 = np.array([right_agent_z - agent_z])
                if agent_types[agent_id] != 0 : # spot
                    ...
                    for k in nearest_goal:
                        ...
                        obs_4 = np.array([0.0]) # goal z
                        obs_5 = np.array([0.0]) # goal dir
                        obs_6 = np.array([0.0]) # goal speed
                else: # goal
                    agent_x = self.global_state[_LOC_X][self.timestep][agent_id]
                    agent_y = self.global_state[_LOC_Y][self.timestep][agent_id]
                    dir_agent = self.global_state[_DIR][self.timestep][agent_id]
                    total_dis = 50
                    #ic(self.stair_position)
                    for stair in range(self.num_stairs):
                        
                        down_diff_x = self.stair_position[stair * 4 ] - agent_x
                        down_diff_y = self.stair_position[stair * 4 + 1] - agent_y
                        down_dis = (down_diff_x ** 2 + down_diff_y ** 2) ** 0.5

                        up_diff_x = self.stair_position[stair * 4 + 2] - agent_x
                        up_diff_y = self.stair_position[stair * 4 + 3] - agent_y
                        up_dis = (up_diff_x ** 2 + up_diff_y ** 2) ** 0.5
                        if down_dis < total_dis or up_dis < total_dis:
                            down_x = down_diff_x * np.cos(dir_agent) + down_diff_y * np.sin(dir_agent)
                            down_y = -1 * down_diff_x * np.sin(dir_agent) + down_diff_y * np.cos(dir_agent)

                            up_x = up_diff_x * np.cos(dir_agent) + up_diff_y * np.sin(dir_agent)
                            up_y = -1 * up_diff_x * np.sin(dir_agent) + up_diff_y * np.cos(dir_agent)
                            if down_dis < up_dis:
                                obs_11 = np.array([down_x])
                                obs_12 = np.array([down_y])
                                obs_13 = np.array([up_x])
                                obs_14 = np.array([up_y])
                            else:
                                obs_11 = np.array([up_x])
                                obs_12 = np.array([up_y])
                                obs_13 = np.array([down_x])
                                obs_14 = np.array([down_y])
    ...
    def get_data_dictionary(self):
        ...
        data_dict.add_data(
            name="loc_x_t_1",
            data=self.global_state[_LOC_X][0],
            save_copy_and_apply_at_reset=True
        )
        data_dict.add_data(
            name="loc_y_t_1",
            data=self.global_state[_LOC_Y][0],
            save_copy_and_apply_at_reset=True
        )
        data_dict.add_data(
            name="loc_x_t_2",
        ...
        data_dict.add_data(name="spot_cooperate", data=self.spot_cooperate)
        data_dict.add_data(name="need_reverse_env", data=self.need_reverse_env)
        data_dict.add_data(name="num_preprocess_feture", data=self.num_preprocess_feture)
        data_dict.add_data(name="car_mode", data=self.car_mode)
        data_dict.add_data(
