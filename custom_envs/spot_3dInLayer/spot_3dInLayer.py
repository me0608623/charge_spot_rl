    ...
    def generate_observation(self):
        ...
        for agent_id in range(self.num_agents):
            if self.timestep == 0:
                ...
                obs_local_grident = 2.0 * np.ones(int(360 / self.spot_lidar_angle), dtype=self.float_dtype)
            ...
            agent_grid_x = min(int(self.global_state[_LOC_X][self.timestep][agent_id] / self.grid_density + 0.5), self.map_max_x - 1)
            agent_grid_y = min(int(self.global_state[_LOC_Y][self.timestep][agent_id] / self.grid_density + 0.5), self.map_max_x - 1)
            self_agent_id = self.map_max_x * agent_grid_y + agent_grid_x
            agent_z = self.grid_map_z_[self_agent_id]
            grident_distance = 1.0
            for k in range(self.num_angle_lidar):
                lidar_angle = self.global_state[_DIR][self.timestep][agent_id] + k * self.spot_lidar_angle * np.pi / 180.0
                lidar_x_dir = np.cos(lidar_angle)
                lidar_y_dir = np.sin(lidar_angle)
                lidar_grid_x = min(max(int((self.global_state[_LOC_X][self.timestep][agent_id] + lidar_x_dir * grident_distance) / self.grid_density + 0.5), 0), self.map_max_x - 1)
                lidar_grid_y = min(max(int((self.global_state[_LOC_Y][self.timestep][agent_id] + lidar_y_dir * grident_distance) / self.grid_density + 0.5), 0), self.map_max_x - 1)
                lidar_z = self.grid_map_z_[int(self.map_max_x * lidar_grid_y + lidar_grid_x)] - agent_z
                obs_local_grident[k] = lidar_z

            obs_0 = np.array([self.global_state[_SP_X][self.timestep][agent_id] / (self.spot_max_speed_x + self.eps)])
            obs_1 = np.array([0.0])
            obs_2 = np.array([0.0])
            obs_7 = time
            if self.still_in_the_game[agent_id]:  obs_8 = np.array([1.0])
            ...
            if agent_types[agent_id] != 0 : # spot
                ...
                obs_3 = np.array([front_agent_z - agent_z])
                obs_4 = np.array([back_agent_z - agent_z])
                obs_5 = np.array([left_agent_z - agent_z])
                obs_6 = np.array([right_agent_z - agent_z])
