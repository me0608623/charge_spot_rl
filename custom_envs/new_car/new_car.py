...
class New_Car(CUDAEnvironmentContext):
    """
        real car info 
        fsp = 10
        acc_max: 1.0 m/s fit
    ...
    def __init__(
        ...
        car_max_speed_x = 0.8, 
        car_min_speed_x = -0.2,
        car_max_acceleration_x = 0.2,
        car_min_acceleration_x = -0.1,
        car_max_turn_pi_ = 15.0, # max_turn = np.pi / 15
        car_reward_get_goal = 20.0,
        car_floor_direction_reward = 0.05,
        base_floor_direction_reward = 0.05,
        car_cost_operate = 0.05,
        car_cost_turn_rate_with_acc_x = 0.6,
        car_penalty_hit = -15.0,
        car_action_levels = 20,
        car_size_bias = 0.15,
        car_size_rand = 0.2,
        reward_on_floor = 0.1,
        ...
        goal_size = 0.1,
        goal_speed_rate_with_car = 0.5,
        obstacle_speed_rate_with_car = 1,
        Testing_type = 1,
        ...
        if self.Training == True:
            ...
            if num_car != 1 or num_goals != 1 :
                print("testing number car and goal = 1 ")
            num_car = 1
            num_goals = 1
        elif Testing_type == 2:
        ...
        if 360 % car_lidar_angle != 0:
            print("car_lidar_angle slould be 360 common factor .")
        assert 360 % car_lidar_angle == 0
        self.car_lidar_angle = self.int_dtype(car_lidar_angle)
        self.car_lidar_angle_f = self.float_dtype(car_lidar_angle)

        self.num_car = num_car  
        self.num_obstacle = num_obstacle
        self.num_goals = num_goals
        self.num_agents = self.num_car + self.num_obstacle + self.num_goals
        ...
        for agent_id in range(self.num_agents):
            if agent_id in set(no_car_):
                ...
                self.agent_type[agent_id] = 1  # cars
        ...
        # Set the speed limit
        self.car_max_speed_x = self.float_dtype(car_max_speed_x)
        self.car_min_speed_x = self.float_dtype(car_min_speed_x)

        # Set the acceleration limit
        self.car_max_acceleration_x = self.float_dtype(car_max_acceleration_x)
        self.car_min_acceleration_x = self.float_dtype(car_min_acceleration_x)

        # Set the turn limit
        self.car_max_turn = self.float_dtype(np.pi / car_max_turn_pi_)
        self.car_min_turn = self.float_dtype(-1 * np.pi / car_max_turn_pi_)
        ...
        # Set Action
        assert car_action_levels >= 0
        self.car_action_levels = car_action_levels

        # Acceleration actions
        self.acceleration_actions_x = np.linspace(
            self.car_min_acceleration_x, self.car_max_acceleration_x, self.car_action_levels
        )
        self.skill_levels = [self.float_dtype(goal_speed_rate_with_car) if self.agent_type[agent_id] == 0 else self.float_dtype(1.0) if self.agent_type[agent_id] == 1 else self.float_dtype(obstacle_speed_rate_with_car) for agent_id in range(self.num_agents)]

        # Turn actions
        self.turn_actions = np.linspace(
            self.car_min_turn, self.car_max_turn, self.car_action_levels
        )
        ...
        """
        display_reward = 
            goal(5) dynamic_Obstacle Obstacle car Step Move Floor
            car(3) dynamic_Obstacle Obstacle Goal dynamic floor
            一半紀錄
        """
        ...
        # car
        self.car_penalty_hit = self.float_dtype(car_penalty_hit)
        self.car_reward_get_goal = self.float_dtype(car_reward_get_goal)
        self.car_floor_direction_reward = self.float_dtype(car_floor_direction_reward)
        self.base_floor_direction_reward = self.float_dtype(base_floor_direction_reward)
        
        self.car_cost_operate = self.float_dtype(car_cost_operate)
        self.car_cost_turn_rate_with_acc_x = self.float_dtype(car_cost_turn_rate_with_acc_x)
        ...
        self.cars_at_reset = copy.deepcopy(self.cars)
        ...
        floor_width = floor_width_bias + floor_width_rand * random.random()
        ...
        if floor_width < stairs_weight: floor_width = stairs_weight
        ...
        x = np.arange(x_range[0], x_range[1], grid_density)
        y = np.arange(y_range[0], y_range[1], grid_density)
        ...
        edge_left = True
        edge_right = True
        edge_probability = (self.PHASE) * 0.25
        if random.random() > edge_probability: edge_up = False
        if random.random() > edge_probability: edge_down = False
        if random.random() > edge_probability: edge_left = False
        if random.random() > edge_probability: edge_right = False
        ...
        for i in range(len_y):
            for j in range(len_x):
                ...
                if (i == 0 and edge_up) or (j == 0 and edge_left) or (i == len_y - 1 and edge_down) or (j == len_x - 1 and edge_right):
                    self.grid_map_z[i,j] = self.float_dtype(grid_length_height)
        
        self.grid_map_z_ = self.grid_map_z.reshape((len_x * len_y))

        # 計算 3D 地圖每個點每個角度看出去的距離
        self.different_point_angle_distance = np.zeros((int(len_x * len_y * 360 / self.car_lidar_angle)), dtype=self.float_dtype)
        for i in range(len_y):
            for j in range(len_x):
                this_point_origin_z = self.grid_map_z[i,j]
                self.cheak_point_distance(this_point_origin_z, i, j)
                diff_i = abs(self.grid_map_z[min(i + 1, len_y - 1), j] - self.grid_map_z[i, j])
                diff_j = abs(self.grid_map_z[i, min(j + 1, len_x - 1)] - self.grid_map_z[i, j])
    ...
    def map_dx(self, map_x, map_y):
        ...
        elif map_x >= self.max_j - 2: return self.grid_length_height
        elif map_y >= 0 and map_y < self.max_j: return 0.5 * (self.grid_map_z[map_y, map_x + 1] - self.grid_map_z[map_y, map_x - 1])
        else: return 0

    def map_dy(self, map_x, map_y):
        if map_y < 1: return -1 * self.grid_length_height
        elif map_y >= self.max_j - 2: return self.grid_length_height
        elif map_x >= 0 and map_x < self.max_j: return 0.5 * (self.grid_map_z[map_y + 1, map_x] - self.grid_map_z[map_y - 1, map_x])
        else: return 0

    def cheak_point_distance(self, this_point_origin_z, point_y, point_x):
        num_angle_lidar = int(360 / self.car_lidar_angle)
        ...
        for k in range(num_angle_lidar):
            ...
            for len_ in range(max_distance):
                ...
                x_ = int(point_x + np.cos(np.radians(k * self.car_lidar_angle)) * len + 0.5)
                y_ = int(point_y + np.sin(np.radians(k * self.car_lidar_angle)) * len + 0.5)
                if x_ > self.max_i - 1 or x_ < 0 or y_ > self.max_j - 1 or y_ < 0:
                    break
                if abs(last_z - self.grid_map_z[y_, x_]) > 0.3:
                    break
                #if abs(last_z - this_point_origin_z) > 0.3:
                    # 不同樓層的距離比較短
                    #on_stair_len += len_grid_precision
                last_z = self.grid_map_z[y_, x_]
            self.different_point_angle_distance[num_angle_lidar * self.max_j * point_y + num_angle_lidar * point_x + k] =  self.float_dtype((len - len_grid_precision) * self.grid_density - 0.5 * (on_stair_len) * self.grid_density)
        return 0
    ...
    def update_state(self, delta_accelerations_x, delta_turns):
        """
        """
        ...
        max_speed_x = self.car_max_speed_x * np.array(self.skill_levels)
        min_speed_x = self.car_min_speed_x * np.array(self.skill_levels)
        ...
        loc_x_curr_t = self.float_dtype(
            loc_x_prev_t + speed_curr_t_x * np.cos(dir_curr_t)
        )
        loc_y_curr_t = self.float_dtype(
            loc_y_prev_t + speed_curr_t_x * np.sin(dir_curr_t)
        )
    ...
    def compute_hit_wall(self, agent_x, agent_y, agent_size):
        ...
        for grid_map_y in range(len(self.grid_map_z)):
            for grid_map_x in range(len(self.grid_map_z[grid_map_y])):
                if abs(np.sqrt((agent_grid_x - grid_map_x) ** 2 + (agent_grid_y - grid_map_y) ** 2) - agent_size / self.grid_density) < 0.5:
                    if abs(self.grid_map_z[agent_grid_y, grid_map_x] - self.grid_map_z[agent_grid_y, grid_map_x + 1]) > self.grid_density * 1.2 / 1.732:  return 1
                    if abs(self.grid_map_z[agent_grid_y, grid_map_x]  - self.grid_map_z[agent_grid_y + 1, grid_map_x]) > self.grid_density * 1.2 / 1.732:  return 1
        return 0
    ...
    def compute_hit_wall_angle_base(self, agent_x, agent_y, agent_size):
        ...
        left_new_agent_grid_x = min(int(agent_x / self.grid_density), self.max_j - 1)
        down_new_agent_grid_y = min(int(agent_y / self.grid_density), self.max_j - 1)
        ...
        num_angle_lidar = 360 / self.car_lidar_angle
        for angle in range(num_angle_lidar):
            left_down_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * down_new_agent_grid_y + num_angle_lidar * left_new_agent_grid_x + angle]
            left_up_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * up_new_agent_grid_y + num_angle_lidar * left_new_agent_grid_x + angle]
            right_down_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * down_new_agent_grid_y + num_angle_lidar * right_new_agent_grid_x + angle]
            right_up_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * up_new_agent_grid_y + num_angle_lidar * right_new_agent_grid_x + angle]
    ...
    def generate_observation(self):
        ...
        for feature in [
            (_LOC_X, 1),
            (_LOC_Y, 1),
            (_SP_X, self.car_max_speed_x + self.eps),
            (_DIR, 2 * np.pi),
        ]:
        ...
        for agent_id in range(self.num_agents):
            if self.timestep == 0:
                ...
                obs_local_angle = np.zeros(int(360 / self.car_lidar_angle), dtype=self.float_dtype)
            ...
            if self.still_in_the_game[agent_id]:
                ...
                num_angle_lidar = int(360 / self.car_lidar_angle)
                angle_ = int(0.5 + self.global_state[_DIR][self.timestep][agent_id] * 360 / (2 * np.pi) / self.car_lidar_angle)
                left_new_agent_grid_x = min(int(self.global_state[_LOC_X][self.timestep][agent_id] / self.grid_density), self.max_j - 1)
                down_new_agent_grid_y = min(int(self.global_state[_LOC_Y][self.timestep][agent_id] / self.grid_density), self.max_j - 1)
                right_new_agent_grid_x = min(left_new_agent_grid_x + 1, self.max_j - 1)
                up_new_agent_grid_y = min(down_new_agent_grid_y + 1, self.max_j - 1)
                ...
                for k in range(num_angle_lidar):
                    ...
                    left_down_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * down_new_agent_grid_y + num_angle_lidar * left_new_agent_grid_x + real_angle]
                    left_up_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * up_new_agent_grid_y + num_angle_lidar * left_new_agent_grid_x + real_angle]
                    right_down_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * down_new_agent_grid_y + num_angle_lidar * right_new_agent_grid_x + real_angle]
                    right_up_angle_distance = self.different_point_angle_distance[num_angle_lidar * self.max_j * up_new_agent_grid_y + num_angle_lidar * right_new_agent_grid_x + real_angle]
                    precise_angle_distance = precise_new_agent_grid_x * precise_new_agent_grid_y * right_up_angle_distance + (1 - precise_new_agent_grid_x) * precise_new_agent_grid_y * left_up_angle_distance + precise_new_agent_grid_x * (1 - precise_new_agent_grid_y) * right_down_angle_distance + (1 - precise_new_agent_grid_x) * (1 - precise_new_agent_grid_y) * left_down_angle_distance
                    max_angle_distance = max(max(left_down_angle_distance, left_up_angle_distance), max(right_down_angle_distance, right_up_angle_distance))
                    min_angle_distance = min(min(left_down_angle_distance, left_up_angle_distance), min(right_down_angle_distance, right_up_angle_distance))
                    smallest_distance = min(min(precise_new_agent_grid_x, 1 - precise_new_agent_grid_x), min(precise_new_agent_grid_y, 1 - precise_new_agent_grid_y))
                ...
                if agent_types[agent_id] != 1 :
                    ...
                    agent_map_x = min(int(self.global_state[_LOC_X][self.timestep][agent_id] / self.grid_density + 0.5), self.max_j - 1)
                    agent_map_y = min(int(self.global_state[_LOC_Y][self.timestep][agent_id] / self.grid_density + 0.5), self.max_j - 1)
                    agent_map_z = self.grid_map_z[agent_map_y, agent_map_x]
                    for k in nearest_neighbor_ids:
                        if agent_types[k] == 0:
                            ...
                            other_map_x = min(int(self.global_state[_LOC_X][self.timestep][k] / self.grid_density + 0.5), self.max_j - 1)
                            other_map_y = min(int(self.global_state[_LOC_Y][self.timestep][k] / self.grid_density + 0.5), self.max_j - 1)
                            other_map_z = self.grid_map_z[other_map_y, other_map_x]
                ...
                obs[agent_id] = np.concatenate(
                    [
                        np.array([self.global_state[_SP_X][self.timestep][agent_id] / (self.car_max_speed_x + self.eps)]),
    ...
    def compute_reward(self):
        ...
        cars_locations_x = self.global_state[_LOC_X][self.timestep][cars_list]
        ...
        cars_locations_y = self.global_state[_LOC_Y][self.timestep][cars_list]

        cars_to_obstacles_distances = np.sqrt(
            (
                np.repeat(cars_locations_x, self.num_obstacle)
                - np.tile(obstacles_locations_x, self.num_car)
            )
            ** 2
            + (
                np.repeat(cars_locations_y, self.num_obstacle)
                - np.tile(obstacles_locations_y, self.num_car)
            )
            ** 2
        ).reshape(self.num_car, self.num_obstacle)
        ...
        goals_to_cars_distances = np.sqrt(
            (
                np.repeat(goals_locations_x, self.num_car)
                - np.tile(cars_locations_x, self.num_goals)
            )
            ** 2
            + (
                np.repeat(goals_locations_y, self.num_car)
                - np.tile(cars_locations_y, self.num_goals)
            )
            ** 2
        ).reshape(self.num_goals, self.num_car)

        min_cars_to_obstacles_distances = np.min(
            cars_to_obstacles_distances, axis=1
        )
        argmin_cars_to_obstacles_distances = np.argmin(
            cars_to_obstacles_distances, axis=1
        )
        nearest_obstacles_cars_ids = [
            obstacles_list[idx] for idx in argmin_cars_to_obstacles_distances
        ]
        ...
        min_goals_to_cars_distances = np.min(
            goals_to_cars_distances, axis=1
        )
        argmin_goals_to_cars_distances = np.argmin(
            goals_to_cars_distances, axis=1
        )
        nearest_cars_goals_ids = [
            cars_list[idx] for idx in argmin_goals_to_cars_distances
        ]

        for idx, cars_id in enumerate(cars_list):
            # 動態障礙物
            if min_cars_to_obstacles_distances[idx] < self.agent_size[idx] + self.agent_size[nearest_obstacles_cars_ids[idx]]:
                rew[cars_id] += float(self.car_penalty_hit)
                self.still_in_the_game[cars_id] = 0
                self.num_hit_obstacle += 1
                del self.cars[cars_id]
                self.num_car -= 1

            # 靜態障礙物   
            compute_hit_wall_ = self.compute_hit_wall_angle_base(
                self.global_state[_LOC_X][self.timestep][cars_id],
                self.global_state[_LOC_Y][self.timestep][cars_id],
                self.agent_size[cars_id]
            )
            ...
            # 運動模型
            if self.PHASE > 1:
                nomal_acc_x = abs(self.global_state[_ACC_X][self.timestep][cars_id]) / max(self.car_max_acceleration_x, abs(self.car_min_acceleration_x))

                nomal_turn_ = abs(self.global_state[_DIR][self.timestep][cars_id] - self.global_state[_DIR][self.timestep - 1][cars_id] ) / self.car_max_turn
                rew[cars_id] += self.car_cost_operate * (1 - nomal_acc_x) ** 2
                rew[cars_id] += self.car_cost_operate * self.car_cost_turn_rate_with_acc_x * (1 - nomal_turn_) ** 2
            else:
                if self.global_state[_SP_X][self.timestep][cars_id] > 0:
                    rew[cars_id] += self.car_cost_operate * 20

            # Floor reward
            nomal_speed_x = abs(self.global_state[_SP_X][self.timestep][cars_id]) / self.car_max_speed_x
            agent_grid_x = min(int(self.global_state[_LOC_X][self.timestep][cars_id] / self.grid_density + 0.5), self.max_j - 1)
            agent_grid_y = min(int(self.global_state[_LOC_Y][self.timestep][cars_id] / self.grid_density + 0.5), self.max_j - 1)
            self_agent_id = self.max_j * agent_grid_y + agent_grid_x
            if abs(self.grid_map_z_[self_agent_id]) > 0 and abs(self.grid_map_z_[self_agent_id]) < self.grid_length_height:
                rew[cars_id] += self.car_floor_direction_reward * nomal_speed_x
        ...
        for idx, goal_id in enumerate(goals_list):
            ...
            if min_goals_to_obstacles_distances < self.agent_size[idx] + self.agent_size[nearest_obstacles_goals_ids[idx]]:
                rew[goal_id] += float(self.car_penalty_hit)
                hit_goal = True
            ...
            # car reward
            if min_goals_to_cars_distances[idx] < self.agent_size[idx] + self.agent_size[nearest_cars_goals_ids[idx]]:
                rew[goal_id] += float(self.car_penalty_hit)
                rew[nearest_cars_goals_ids[idx]] += self.car_reward_get_goal
            ...
            # Move reward
            goal_max_speed = self.skill_levels[goal_id] * self.car_max_speed_x
            goal_max_acceleration = self.skill_levels[goal_id] * self.car_max_acceleration_x
            ...
            # Floor reward
            agent_grid_x = min(int(self.global_state[_LOC_X][self.timestep][goal_id] / self.grid_density + 0.5), self.max_j - 1)
            agent_grid_y = min(int(self.global_state[_LOC_Y][self.timestep][goal_id] / self.grid_density + 0.5), self.max_j - 1)
    ...
    def get_data_dictionary(self):
        ...
        data_dict = DataFeed()
        for feature in [_LOC_X, _LOC_Y, _SP_X, _DIR, _ACC_X]:
            data_dict.add_data(
                name=feature,
        ...
        data_dict.add_data(name="car_lidar_angle", data=self.car_lidar_angle_f)
        data_dict.add_data(name="car_max_turn", data=self.car_max_turn)
        data_dict.add_data(name="car_max_speed_x", data=self.car_max_speed_x)
        data_dict.add_data(name="car_min_speed_x", data=self.car_min_speed_x)
        data_dict.add_data(name="car_max_acceleration_x", data=self.car_max_acceleration_x)
        data_dict.add_data(name="car_min_acceleration_x", data=self.car_min_acceleration_x)
        data_dict.add_data(name="car_reward_get_goal", data=self.car_reward_get_goal)
        data_dict.add_data(name="car_floor_direction_reward", data=self.car_floor_direction_reward)
        data_dict.add_data(name="base_floor_direction_reward", data=self.base_floor_direction_reward)
        data_dict.add_data(name="car_penalty_hit", data=self.car_penalty_hit)
        data_dict.add_data(name="car_cost_operate", data=self.car_cost_operate)
        data_dict.add_data(name="car_cost_turn_rate_with_acc_x", data=self.car_cost_turn_rate_with_acc_x)
    ...
    def reset(self):
        ...
        self.cars = copy.deepcopy(self.cars_at_reset)
        self.num_car = len(self.cars)
    ...
    def step(self, actions=None):
        ...
        if self.use_cuda:
            ...
            args = [
                ...
                "car_max_speed_x",
                "car_min_speed_x",
                "car_max_acceleration_x",
                "car_min_acceleration_x",
                "car_reward_get_goal",
                "car_floor_direction_reward",
                "base_floor_direction_reward",
                "car_penalty_hit",
                "car_cost_operate",
                "car_cost_turn_rate_with_acc_x",
                "goals_reward_step",
                "goals_reward_speed",
                "reward_on_floor",
                "grid_length_max_y",
            ...
            assert all(0 <= acc_x <= self.car_action_levels for acc_x in acceleration_x_action_ids)
            assert all(0 <= turn <= self.car_action_levels for turn in turn_action_ids)
            ...
            self.update_state(delta_accelerations_x, delta_turns)
