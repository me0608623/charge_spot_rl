...
class Spot_3d(CUDAEnvironmentContext, Map):
    ...
    def __init__(
        ...
        self.num_obstacle = int(num_obstacle * random.random() + 1)
    ...
    def generate_observation(self):
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.still_in_the_game[agent_id]:
                ...
                distace_angle = np.arctan2(diff_y, diff_x)
                if self.agent_type[agent_id] == 0:
                    agent_view_global_obs_[0,nearest_neighbor_ids] = diff_x
                    agent_view_global_obs_[1,nearest_neighbor_ids] = diff_y
                else:
                    agent_view_global_obs_[0,nearest_neighbor_ids] = distances_from_origin
                    agent_view_global_obs_[1,nearest_neighbor_ids] = distace_angle
                ...
                if agent_types[agent_id] != 0 : # spot
                    ...
                    for k in nearest_goal:
                        ...
                        distace_angle = math.atan2(agent_view_y, agent_view_x)
                        obs_2 = np.array([distace_]) # goal x
                        obs_3 = np.array([distace_angle]) # goal y
