...
extern "C" {
  ...
  __device__ void CudaTagContinuousGenerateObservation(
    ...
    if (kThisAgentId < kNumAgents) {
        ...
        obs_arr[kThisAgentIdxOffset + 7] = 0.0;  // close front pitch
        obs_arr[kThisAgentIdxOffset + 8] = 0.0;  // close back pitch
        obs_arr[kThisAgentIdxOffset + 9] = 0.0;  // close left pitch
        obs_arr[kThisAgentIdxOffset + 10] = 0.0;  // close right pitch
        obs_arr[kThisAgentIdxOffset + 11] = 0.0;  // time
        obs_arr[kThisAgentIdxOffset + 12] = 0.0;  // in gmae
        obs_arr[kThisAgentIdxOffset + 13] = 0.0;  // front pitch
        obs_arr[kThisAgentIdxOffset + 14] = 0.0;  // back pitch
        obs_arr[kThisAgentIdxOffset + 15] = 0.0;  // left pitch
        obs_arr[kThisAgentIdxOffset + 16] = 0.0;  // right pitch
        ...
        if (still_in_the_game_arr[kThisAgentArrayIdx]) {
          ...
          for (int idx = 0; idx < min(num_valid_other_agents, kNumOtherAgentsObserved); idx++) {
            ...
            float distace_ = pow(pow(dif_x, 2) + pow(dif_y, 2), 0.5);
            float distace_angle = atan2(obs_spot_y_, obs_spot_x_);
            if (agent_types_arr[kThisAgentId] == 0) //goal obs
            {
              distace_ = obs_spot_x_;
              distace_angle = obs_spot_y_;
            }
            obs_arr[kThisAgentIdxOffset + 17 + 0 * kNumOtherAgentsObserved + idx] = static_cast<float>(distace_);
            obs_arr[kThisAgentIdxOffset + 17 + 1 * kNumOtherAgentsObserved + idx] = static_cast<float>(distace_angle);
          ...
          else
          {
            ...
            float distace_angle = atan2(goal_spot_y_, goal_spot_x_);
            obs_arr[kThisAgentIdxOffset + 2] =  static_cast<float>(distace_);
            obs_arr[kThisAgentIdxOffset + 3] =  static_cast<float>(distace_angle);
          ...
          obs_arr[kThisAgentIdxOffset + 11] = static_cast<float>(env_timestep_arr[kEnvId]) / kEpisodeLength;
          obs_arr[kThisAgentIdxOffset + 12] = static_cast<float>(1);
          obs_arr[kThisAgentIdxOffset + 13] = static_cast<float>(front_z);
          obs_arr[kThisAgentIdxOffset + 14] = static_cast<float>(back_z);
          obs_arr[kThisAgentIdxOffset + 15] = static_cast<float>(left_z);
          obs_arr[kThisAgentIdxOffset + 16] = static_cast<float>(right_z);
  ...
  __global__ void CudaSpot_3dDisStep(
    int * done_arr,
    int * env_timestep_arr,
    int * num_spot,
    int * num_hit_obstacle,
