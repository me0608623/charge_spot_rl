...
extern "C" {
      ...
      if (kUseFullObservation) {
        ...
        other_agent_id++) {
          if (other_agent_id != kThisAgentId) {
            ...
            obs_arr[kThisAgentIdxOffset + 7 * (kNumAgents - 1) + index]
              = agent_size_arr[kEnvId * kNumAgents + other_agent_id];
            index += 1;
          }
        }
        ...
        const int kThisAgentIdxOffset = kEnvId * kNumAgents *
          (num_features * kNumOtherAgentsObserved + 8) +
          kThisAgentId * (num_features * kNumOtherAgentsObserved + 8);
        for (int idx = 0; idx < kNumOtherAgentsObserved; idx++) {
          ...
          obs_arr[kThisAgentIdxOffset + 6 * kNumOtherAgentsObserved + idx] =
            0.0;
          obs_arr[kThisAgentIdxOffset + 7 * kNumOtherAgentsObserved + idx] =
            0.0;
        }
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 1] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 2] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 3] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 4] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 5] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 6] =
            0.0;
        obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 7] =
            0.0;
        ...
        if (still_in_the_game_arr[kThisAgentArrayIdx]) {
          ...
          for (int idx = 0; idx < min(num_valid_other_agents,
            ...
            obs_arr[kThisAgentIdxOffset + 7 * kNumOtherAgentsObserved + idx] =
              agent_size_arr[kOtherAgentId] + agent_size_arr[kThisAgentId];
          }
          obs_arr[
            kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved] =
            static_cast<float>(env_timestep_arr[kEnvId]) / kEpisodeLength;
          obs_arr[
            kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 1] =
            agent_size_arr[kThisAgentId];  

          obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 2] = 0;  
          obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 3] = 0;  
          obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 4] = 0;  
          obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 5] = 0;  
          obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 6] = 0;  
          obs_arr[kThisAgentIdxOffset + num_features * kNumOtherAgentsObserved + 7] = 0;            
        }
      }
    }
  }
  ...
  __device__ void CudaTagContinuousComputeReward(
    ...
    const float kDistanceMarginForReward2,
    const float kDistanceMarginForPenalty,
    const float kTagRewardForTagger,
    const float kHitTaggerForTaggerPenalty,
    const float kTagPenaltyForWall,
    const float kTagPenaltyForGoal,
    const float kEndOfGameRewardForWall,
    const float kEndOfGameRewardForGoal,
    const float MaxAcceleration,
    const float MaxTurn,
    const float kMaxSpeed,
    bool kWallExitsGameAfterTagged,
    bool kGoalExitsGameAfterTagged,
    int * still_in_the_game_arr,
    int * done_arr,
    int * env_timestep_arr,
    int kNumAgents,
    ...
    if (kThisAgentId < kNumAgents) {
      ...
      float people_wall_dist = kGridLength * sqrt(2.0);
      bool is_goal = agent_types_arr[kThisAgentId] == 1 ;
      bool not_tagger = agent_types_arr[kThisAgentId] != 2;
      ...
      if (still_in_the_game_arr[kThisAgentArrayIdx]) {
        ...
        rewards_arr[kThisAgentArrayIdx] += 1.3 * kLimitAccelerationReward * speed_arr[kThisAgentArrayIdx] / kMaxSpeed;
        rewards_arr[kThisAgentArrayIdx] += kLimitAccelerationReward * abs(MaxAcceleration - acceleration_arr[kThisAgentArrayIdx])/ MaxAcceleration;
        if (is_wall != 0){
          rewards_arr[kThisAgentArrayIdx] += kLimitTurnReward * abs(MaxTurn - delta_turn)/ kTwoPi;
        }
      }



      if (not_tagger && still_in_the_game_arr[kThisAgentArrayIdx]) {
        int nearest_tagger_id;
        int nearest_wall_id;
        for (int other_agent_id = 0; other_agent_id < kNumAgents;
          other_agent_id++) {
          bool is_tagger = (agent_types_arr[other_agent_id] == 2);
          bool other_is_wall = (agent_types_arr[other_agent_id] == 0);

          if (is_tagger) {
            //agent goal or wall other tagger
          ...
          else if (other_is_wall && is_goal){
            //agent goal other wall
            const float dist = ComputeDistance(
              agent_size_arr,
              loc_x_arr,
              loc_y_arr,
              ...
              kEnvId,
              kNumAgents);
            if (dist < people_wall_dist) {
              people_wall_dist = dist;
            }
          }
        }

        if (min_dist < kDistanceMarginForReward2) {
          // the runner is tagged.
          if (is_goal)
          {
            //agent goal other tagger
            rewards_arr[kThisAgentArrayIdx] += kTagPenaltyForGoal;
            rewards_arr[kEnvId * kNumAgents + nearest_tagger_id] += kTagRewardForTagger;
            still_in_the_game_arr[kThisAgentArrayIdx] = 0;
            num_goals_arr[kEnvId] -= 1;
          }
          else if (min_dist < kDistanceMarginForReward) {
            //agent wall other tagger
            rewards_arr[kEnvId * kNumAgents + nearest_tagger_id] += 2 * kHitTaggerForTaggerPenalty * speed_arr[kEnvId * kNumAgents + nearest_tagger_id] / kMaxSpeed;
            if (kWallExitsGameAfterTagged) {
              still_in_the_game_arr[kThisAgentArrayIdx] = 0;
              num_wall_arr[kEnvId] -= 1;
            } 
          }

        }
        if (people_wall_dist < kDistanceMarginForReward){
          //agent goal other wall
          rewards_arr[kThisAgentArrayIdx] += kHitTaggerForTaggerPenalty;
        }

        // Add end of game reward for runners at the end of the episode.
        if (env_timestep_arr[kEnvId] == kEpisodeLength && is_goal) {
          rewards_arr[kThisAgentArrayIdx] += kEndOfGameRewardForGoal;
        }
      }
      else if(not_tagger == 0)
      {
        //agent tagger
        int nearest_tagger_id;
        for (int other_agent_id = 0; other_agent_id < kNumAgents;
          other_agent_id++) {
          bool is_tagger = (agent_types_arr[other_agent_id] == 2);
          if (is_tagger && other_agent_id != kThisAgentId) {
            const float dist = ComputeDistance(
              agent_size_arr,
              loc_x_arr,
              loc_y_arr,
        ...
        if (min_dist < kDistanceMarginForPenalty){
          //agent tagger other tagger
          rewards_arr[kThisAgentArrayIdx] += 2 * kHitTaggerForTaggerPenalty * (speed_arr[kEnvId * kNumAgents + nearest_tagger_id] +  speed_arr[kThisAgentArrayIdx]) / kMaxSpeed;
        }
      }
  ...
  __global__ void CudaSagContinuousStep(
    float * loc_x_arr,
    float * loc_y_arr,
    float * speed_arr,
    float * direction_arr,
    ...
    const bool kWallExitsGameAfterTagged,
    const bool kGoalExitsGameAfterTagged,
    int * still_in_the_game_arr,
    const bool kUseFullObservation,
    float * obs_arr,
    ...
    CudaTagContinuousComputeReward(
      ...
      kDistanceMarginForReward2,
      kDistanceMarginForPenalty,
      kTagRewardForTagger,
      kHitTaggerForTaggerPenalty,
      kTagPenaltyForWall,
      kTagPenaltyForGoal,
      kEndOfGameRewardForWall,
      kEndOfGameRewardForGoal,
      MaxAcceleration,
      MaxTurn,
      kMaxSpeed,
      kWallExitsGameAfterTagged,
      kGoalExitsGameAfterTagged,
      still_in_the_game_arr,
      done_arr,
      env_timestep_arr,
      kNumAgents,
