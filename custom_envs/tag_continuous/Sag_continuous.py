...
class SagContinuous2(CUDAEnvironmentContext):
    """
    The game of tag on a continuous circular 2D space.
    There are some taggers trying to tag several walls.
    The taggers want to get as close as possible to the wall, while the wall
    wants to get as far away from them as possible.
    Once a wall is tagged, he exits the game if wall_exits_game_after_tagged is True
    ...
    def __init__(
        ...
        hit_car_distance=0.01,
        tag_reward_for_tagger=1.0,
        hit_tagger_for_tagger_penalty=-1.0,
        step_penalty_for_tagger=-0.00,
        tag_penalty_for_wall=-1.0,
        tag_penalty_for_goal=-1.0,
        step_reward_for_wall=0.0,
        step_reward_for_goal=0.03,
        end_of_game_reward_for_wall=1.0,
        end_of_game_reward_for_goal=1.0,
        limit_acceleration_reward=0.02,
        limit_turn_reward=0.02,
        wall_exits_game_after_tagged= False,
        goal_exits_game_after_tagged=True,
        use_cuda=False,
    ):
        """
        Args:
            num_taggers (int, optional): [number of taggers in the environment].
                Defaults to 1.
            num_walls (int, optional): [number of taggers in the environment].
                Defaults to 10.
            ...
            skill_level_wall (float, optional): [wall skill level;
            ...
            tagging_distance (float, optional): [margin between a
                tagger and wall to consider the wall as 'tagged'. This multiplies
                on top of the grid length]. Defaults to 0.01.
            tag_reward_for_tagger (float, optional): [positive reward for the tagger
                upon tagging a wall]. Defaults to 1.0
            step_penalty_for_tagger (float, optional): [penalty for every step
                the game goes on]. Defaults to -0.0.
            tag_penalty_for_wall (float, optional): [negative reward for getting
                tagged]. Defaults to -1.0
            step_reward_for_wall (float, optional): [reward for every step the
                wall isn't tagged]. Defaults to 0.0.
            end_of_game_reward_for_wall (float, optional): [reward at the end of
                the game for a wall that isn't tagged]. Defaults to 1.0.
            wall_exits_game_after_tagged (bool, optional): [boolean indicating
                whether walls exit the game after getting tagged or can remain in and
        ...
        self.num_agents = self.num_taggers + self.num_walls + self.num_goals
        ...
        walls = self.np_random.choice(
            np.arange(self.num_agents), self.num_walls + self.num_goals, replace=False
        )
        walls1 = self.np_random.choice(
            walls, num_walls, replace=False
        )
        ...
        self.skill_levels = [self.float_dtype(skill_level_wall * random.random()) if self.agent_type[agent_id] ==0 else self.float_dtype(skill_level_goal) if self.agent_type[agent_id] == 1 else self.float_dtype(skill_level_tagger) for agent_id in range(self.num_agents)]

        # Does the wall exit the game or continue to play after getting tagged?
        self.wall_exits_game_after_tagged = wall_exits_game_after_tagged
        self.goal_exits_game_after_tagged = goal_exits_game_after_tagged
        ...
        self.distance_margin_for_reward2 = (tagging_distance * self.grid_length).astype(
            self.float_dtype
        )
        assert 0 <= hit_wall_distance <= tagging_distance
        self.distance_margin_for_reward = (hit_wall_distance * self.grid_length).astype(
            self.float_dtype
        )
        assert 0 <= hit_car_distance <=1
        self.distance_margin_for_penalty = (hit_car_distance * self.grid_length).astype(
            self.float_dtype
        )
        # Rewards and penalties
        assert tag_reward_for_tagger >= 0
        self.tag_reward_for_tagger = self.float_dtype(tag_reward_for_tagger)
        assert hit_tagger_for_tagger_penalty <= 0
        self.hit_tagger_for_tagger_penalty = self.float_dtype(hit_tagger_for_tagger_penalty)

        assert step_penalty_for_tagger <= 0
        self.step_penalty_for_tagger = self.float_dtype(step_penalty_for_tagger)

        assert tag_penalty_for_wall <= 0
        self.tag_penalty_for_wall = self.float_dtype(tag_penalty_for_wall)
        assert step_reward_for_wall >= 0
        self.step_reward_for_wall = self.float_dtype(step_reward_for_wall)

        assert tag_penalty_for_goal <= 0
        self.tag_penalty_for_goal = self.float_dtype(tag_penalty_for_goal)
        assert step_reward_for_goal >= 0
        self.step_reward_for_goal = self.float_dtype(step_reward_for_goal)

        self.step_rewards =[self.step_penalty_for_tagger if self.agent_type[agent_id]==2 else self.step_reward_for_wall if self.agent_type[agent_id]==0 else self.step_reward_for_goal for agent_id in range(self.num_agents)]

        assert end_of_game_reward_for_wall >= 0
        self.end_of_game_reward_for_wall = self.float_dtype(
            end_of_game_reward_for_wall
        )

        assert end_of_game_reward_for_goal >= 0
        self.end_of_game_reward_for_goal = self.float_dtype(
            end_of_game_reward_for_goal
        )

        self.limit_acceleration_reward = self.float_dtype(
            limit_acceleration_reward
        )
        self.limit_turn_reward = self.float_dtype(
            limit_turn_reward
        )
        ...
        # Copy walls dict for applying at reset
        self.walls_at_reset = copy.deepcopy(self.walls)
        ...
        if self.use_full_observation:
            ...
            for agent_id in range(self.num_agents):
                ...
                if self.still_in_the_game[agent_id]:
                    ...
                    #print("agent_types",nearest_neighbor_ids)
                    obs_still_in_the_game = (
                        np.hstack(
                            (
                    ...
                    obs_agent_size =(
                        np.hstack(
                            (
                                self.agent_size[nearest_neighbor_ids],
                                np.zeros(
                                    (
                                        self.num_other_agents_observed
                                        - len(nearest_neighbor_ids)
    ...
    def compute_reward(self):
        ...
        if self.num_walls > 0:
            ...
            goal_locations_x = self.global_state[_LOC_Y][self.timestep][goals_list]
            tagger_locations_y = self.global_state[_LOC_Y][self.timestep][taggers_list]

            walls_to_taggers_distances = np.sqrt(
                (
                    np.repeat(wall_locations_x, self.num_taggers)
                    - np.tile(tagger_locations_x, self.num_walls)
                )
                ** 2
                + (
                    np.repeat(wall_locations_y, self.num_taggers)
                    - np.tile(tagger_locations_y, self.num_walls)
                )
                ** 2
            ).reshape(self.num_walls, self.num_taggers)
            ...
            taggers_to_taggers_distances = np.sqrt(
                (
                    np.repeat(tagger_locations_x, self.num_taggers)
                    - np.tile(tagger_locations_x, self.num_taggers)
                )
                ** 2
                + (
                    np.repeat(tagger_locations_y, self.num_taggers)
                    - np.tile(tagger_locations_y, self.num_taggers)
                )
                ** 2
            ).reshape(self.num_taggers, self.num_taggers)

            min_walls_to_taggers_distances = np.min(
                walls_to_taggers_distances, axis=1
            )
            argmin_walls_to_taggers_distances = np.argmin(
                walls_to_taggers_distances, axis=1
            )
            nearest_tagger_ids = [
                taggers_list[idx] for idx in argmin_walls_to_taggers_distances
            ]
            ...
            min_taggers_to_taggers_distances2 = np.min(
                taggers_to_taggers_distances, axis=1
            )
            argmin_taggers_to_taggers_distances2 = np.argmin(
                taggers_to_taggers_distances, axis=1
            )
            nearest_tagger_ids3 = [
                taggers_list[idx] for idx in argmin_taggers_to_taggers_distances2
            ]
            #it will be the self distance so it is wrong but it just run 1 time so it find
        ...
        for idx, wall_id in enumerate(walls_list):
            if min_walls_to_taggers_distances[idx] < self.distance_margin_for_reward:

                # the wall is tagged!
                rew[nearest_tagger_ids[idx]] -= self.tag_reward_for_tagger

                if self.wall_exits_game_after_tagged:
                    # Remove wall from game
                    self.still_in_the_game[wall_id] = 0
                    del self.walls[wall_id]
                    self.num_walls -= 1
                    self.global_state[_SIG][self.timestep :, wall_id] = 0

        for idx, goal_id in enumerate(goals_list):
            if min_goal_to_taggers_distances[idx] < self.distance_margin_for_reward2:

                # the wall is tagged!
                rew[goal_id] += self.tag_penalty_for_goal
                rew[nearest_tagger_ids2[idx]] += self.tag_reward_for_tagger
        ...
        """
        for idx, goal_id in enumerate(taggers_list):
            if min_taggers_to_taggers_distances2[idx] < self.distance_margin_for_penalty:
                rew[goal_id] += self.hit_tagger_for_tagger_penalty
        """
        if self.timestep == self.episode_length:
            for wall_id in self.walls:
                rew[wall_id] += self.end_of_game_reward_for_wall
            for wall_id in self.goals:
                rew[wall_id] += self.end_of_game_reward_for_goal
    ...
    def get_data_dictionary(self):
        ...
        data_dict.add_data(
            name="num_walls", data=self.num_walls, save_copy_and_apply_at_reset=True
        )
        data_dict.add_data(
            name="num_goals", data=self.num_goals, save_copy_and_apply_at_reset=True
        )
        data_dict.add_data(
        ...
        data_dict.add_data(
            name="distance_margin_for_reward2", data=self.distance_margin_for_reward2
        )
        data_dict.add_data(
            name="distance_margin_for_penalty", data=self.distance_margin_for_penalty
        )
        data_dict.add_data(
            name="tag_reward_for_tagger", data=self.tag_reward_for_tagger
        )
        data_dict.add_data(
            name="hit_tagger_for_tagger_penalty", data=self.hit_tagger_for_tagger_penalty
        )
        data_dict.add_data(
            name="tag_penalty_for_wall", data=self.tag_penalty_for_wall
        )
        data_dict.add_data(
            name="tag_penalty_for_goal", data=self.tag_penalty_for_goal
        )
        data_dict.add_data(
            name="end_of_game_reward_for_wall",
            data=self.end_of_game_reward_for_wall,
        )
        data_dict.add_data(
            name="end_of_game_reward_for_goal",
            data=self.end_of_game_reward_for_goal,
        )
        data_dict.add_data(
        ...
        data_dict.add_data(
            name="wall_exits_game_after_tagged",
            data=self.wall_exits_game_after_tagged,
        )
        data_dict.add_data(
            name="goal_exits_game_after_tagged",
            data=self.goal_exits_game_after_tagged,
        )
        data_dict.add_data(
    ...
    def reset(self):
        ...
        self.walls = copy.deepcopy(self.walls_at_reset)
    ...
    def step(self, actions=None):
        ...
        if self.use_cuda:
            ...
            args = [
                ...
                "skill_levels",
                "wall_exits_game_after_tagged",
                "goal_exits_game_after_tagged",
                "still_in_the_game",
                ...
                "distance_margin_for_reward2",
                "distance_margin_for_penalty",
                "tag_reward_for_tagger",
                "tag_penalty_for_wall",
                "tag_penalty_for_goal",
                "hit_tagger_for_tagger_penalty",
                "end_of_game_reward_for_wall",
                "end_of_game_reward_for_goal",
                "limit_acceleration_reward",
                "limit_turn_reward",
