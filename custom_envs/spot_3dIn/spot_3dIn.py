...
class Map():
    def __init__(
        ...
        super().__init__()
        self.random_map_seed = np.random
        if seed is not None: self.random_map_seed.seed(seed + 17)
        if random_map == False:  self.random_map_seed.seed(seed + 23)
        else: self.random_map_seed.seed(int(random.random() * 10000))
        self.num_wall = num_wall
        self.wall_length = wall_length
        self.grid_length = grid_length
        self.num_stairs = min(int(self.random_map_seed.rand() * num_stairs) + 1, num_stairs)
    ...
    def generate_map(self):
        ...
        for wall_idx in  range(self.num_wall):
            wall_angle = self.random_map_seed.rand() * 2.0 * np.pi
            origin_x =  self.random_map_seed.rand() * self.grid_length
            origin_y =  self.random_map_seed.rand() * self.grid_length
            wall_info = {
                "start": [
        ...
        for stair_idx in  range(self.num_stairs):
            stairs_weight = self.random_map_seed.rand() * self.stairs_weight_rand + self.stairs_weight_bias
            floor_width = self.random_map_seed.rand() * self.floor_width_rand + self.floor_width_bias
            half_stairs_weight = stairs_weight * 0.5
            half_floor_width = floor_width * 0.5
            origin_x =  (self.random_map_seed.rand() - 0.5) * self.grid_length * 0.8 + grid_length_half
            origin_y =  (self.random_map_seed.rand() - 0.5) * self.grid_length * 0.8 + grid_length_half
            up_or_down_z = True if random.random() > 0.5 else False
            right_or_left = 0 if self.random_map_seed.rand() > 0.5 else 1
            up_or_down_y = -1 if self.random_map_seed.rand() > 0.5 else 1
            stair_info = {
