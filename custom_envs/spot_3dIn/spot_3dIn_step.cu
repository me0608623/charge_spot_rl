...
extern "C" {
  ...
  __device__ void CudaTagContinuousGenerateObservation(
    ...
    if (kThisAgentId < kNumAgents) {
        ...
        const int kThisAgentIdxOffset = kEnvId * kNumAgents *
          (num_features * kNumOtherAgentsObserved + 17 + num_angle_lidar) +
          kThisAgentId * (num_features * kNumOtherAgentsObserved + 17 + num_angle_lidar);
        ...
        for (int idx = 0; idx < kNumOtherAgentsObserved; idx++) {
          ...
          obs_arr[kThisAgentIdxOffset + 17 + 2 * kNumOtherAgentsObserved + idx] = 20.0;  // other next sec LOC_X
          obs_arr[kThisAgentIdxOffset + 17 + 3 * kNumOtherAgentsObserved + idx] = 20.0;  // other next sec LOC_Y
        ...
        for (int angle = 0; angle < num_angle_lidar; angle++){
          obs_arr[kThisAgentIdxOffset + 17 + num_features * kNumOtherAgentsObserved + angle] = 0.0;  // local map infomation
        }
      ...
      else
      {
        ...
        obs_arr[kThisAgentIdxOffset + 7] = static_cast<float>(front_short_z);
        obs_arr[kThisAgentIdxOffset + 8] = static_cast<float>(front_far_z);
        obs_arr[kThisAgentIdxOffset + 9] = static_cast<float>(front_right_z);
        obs_arr[kThisAgentIdxOffset + 10] = static_cast<float>(front_left_z);
        obs_arr[kThisAgentIdxOffset + 11] = static_cast<float>(back_short_z);
        obs_arr[kThisAgentIdxOffset + 12] = static_cast<float>(back_far_z);
        obs_arr[kThisAgentIdxOffset + 13] = static_cast<float>(back_right_z);
        obs_arr[kThisAgentIdxOffset + 14] = static_cast<float>(back_left_z);
      }
      ...
      for (int idx = 0; idx < min(num_agent_state_obs, kNumOtherAgentsObserved); idx++) {
        ...
        float next_dif_x =  static_cast<float>(next_obs_x - loc_x_arr[kThisAgentArrayIdx]);
        float next_dif_y =  static_cast<float>(next_obs_y - loc_y_arr[kThisAgentArrayIdx]);
        float next_obs_spot_x_ = next_dif_x * cos_dir + next_dif_y * sin_dir;
        float next_obs_spot_y_ = - 1 * next_dif_x * sin_dir + next_dif_y * cos_dir;
        ...
        obs_arr[kThisAgentIdxOffset + 17 + 2 * kNumOtherAgentsObserved + idx] = static_cast<float>(next_obs_spot_x_);
        obs_arr[kThisAgentIdxOffset + 17 + 3 * kNumOtherAgentsObserved + idx] = static_cast<float>(next_obs_spot_y_);
        obs_arr[kThisAgentIdxOffset + 17 + 4 * kNumOtherAgentsObserved + idx] = static_cast<float>(0);
      ...
      for (int idx = num_agent_state_obs; idx < num_agent_state_obs + num_static_state_obs ; idx++) {
        ...
        if (agent_types_arr[kThisAgentId] == 1)  // spot
        {
          ...
          float distance_obs_size = obs_size * (real_obs_distance / obs_distance);
          ...
          else
          {
            for (int i = 0 ; i < end_angle + 1; i++)
            {
              ...
              if (abs(diff_angle) > side_angle) diff_angle -= 360;
      ...
      if (still_in_the_game_arr[kThisAgentArrayIdx] || is_goal) {
        ...
        if(is_goal)
        {
          ...
          if(dist_spot <= 0)
          {
            ...
            display_reward[kThisAgentIdxOffset + 2] += 0;
            //goal_hit = true;
          }
  ...
  __global__ void CudaSpot_3dInStep(
    int * done_arr,
    int * env_timestep_arr,
    int * num_spot,
    int * num_hit_obstacle,
    ...
    if (env_timestep_arr[kEnvId] <= 1)
    {
      ...
      {
        loc_x_arr[kThisAgentArrayIdx] = float(int(loc_x_arr[kThisAgentArrayIdx] + 5.02 + float(kEnvId) * 1.3) % int(kGridLength));
        loc_y_arr[kThisAgentArrayIdx] = float(int(loc_y_arr[kThisAgentArrayIdx] + 12.35 + float(kEnvId) * 1.7) % int(kGridLength));
      }
      else if(agent_types_arr[kThisAgentId] == 1 && spot_random_position)
      {

        bool spot_hit = true;
        loc_x_arr[kThisAgentArrayIdx] = float(int(loc_x_arr[kThisAgentArrayIdx] + 5.02 + float(kEnvId) * 0.7) % int(kGridLength));
        ...
        while (spot_hit)
        {
          ...
          bool body_hit = CheckAgentHit(
            ...
            agent_x,
            agent_y,
            agent_size_arr[kThisAgentId] + 0.4,
            num_angle_lidar
          );
