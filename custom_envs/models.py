...
class CustomModelConnected(nn.Module):
    ...
    def __init__(
        ...
        self.rl_input_dim = 512
        if self.n_layers > 0: 
            self.rnn = nn.RNN(rnn_fc_out, self.memory_dim, self.n_layers)
            self.rl_input = nn.Linear(self.memory_dim + flattened_obs_size, self.rl_input_dim)
        else:
            self.rl_input = nn.Linear(rnn_fc_out + flattened_obs_size, self.rl_input_dim)
    ...
    def rnn_process(self, input):
        ...
        if input.dim() == 3:  
            output = input.reshape(1, -1, input.size(-1))
            origin_batch = (input.size(0), input.size(1), -1)
        elif input.dim() == 4: 
            origin_batch = (input.size(0), input.size(1), input.size(2), input.size(3))
            seq_date = input.reshape(-1, self.rnn_seq_len, origin_batch[1], origin_batch[2], origin_batch[3]) 
            seq_date = seq_date.permute(1, 0, 2, 3, 4)
            output = seq_date.reshape(self.rnn_seq_len, -1, origin_batch[3])
        elif input.dim() == 1:
            self.implement = True
            output = input.unsqueeze(0).unsqueeze(0)
            origin_batch = (-1,)
        else:
            ic("rnn input dim error", input.dim())
    ...
    def rnn_recover(self, rout, origin_batch):
        ...
        if len(origin_batch) == 4:
            ...
            seq_date = rout.reshape(self.rnn_seq_len, -1, origin_batch[1], origin_batch[2], rnn_out_dim)
            seq_date = seq_date.permute(1, 0, 2, 3, 4)
    ...
    def forward(self, obs=None, memory=None, batch_index=None, batch_size=None):
        ...
        if self.n_layers > 0:
            ...
            if self.observation_space is None:
                memory_ = memory.unsqueeze(0).unsqueeze(1) 
            elif memory is not None:

                memory_ = memory.reshape(self.n_layers, -1, self.memory_dim) #banch = time, banch1, banch2
            elif self.memory is None and self.observation_space is not None:
                self.count_rnn_seq = 0
                batch_size_ = ip.size(-2) *  ip.size(-3)
                memory_ = torch.zeros(self.n_layers, batch_size_, self.memory_dim).to('cuda:0').detach()
            else:
                memory_ = self.memory
                mem_idx = (self.count_rnn_seq) // self.rnn_seq_len # init self.count_rnn_seq is 1
                if self.count_rnn_seq % self.rnn_seq_len == self.rnn_seq_len - 1 and mem_idx < self.memory_max_index:  
        ...
        if self.count_rnn_seq % self.rnn_seq_len == self.rnn_seq_len - 1:  
            memory_t_1 = memory_reg.detach()
<<<<<<< HEAD
            r_out_detach = r_out.detach()
        else:  
            memory_t_1 = memory_reg 
            r_out_detach = r_out
=======
            r_out_detach = r_out
        else:  
            memory_t_1 = memory_reg 
            r_out_detach = r_out.detach()
>>>>>>> 80375973539c6d689077492feb1fe86bc1c380ca
        ...
        rl_in_ = torch.concat((rl_ip, r_out_detach), dim=-1)
        ...
        if self.training == 'test':
            ...
            for mem_data in range(len(data_dict["memory"])):
                dict_["mem_"+str(mem_data)] = data_dict["memory"][mem_data]
