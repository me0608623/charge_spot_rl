...
extern "C" {
  ...
  __device__ void CudaTagContinuousGenerateObservation(
    ...
    if (kThisAgentId < kNumAgents) {
        ...
        const int kThisAgentIdxOffset = kEnvId * kNumAgents *
          (num_features * kNumOtherAgentsObserved + 18 + num_angle_lidar) +
          kThisAgentId * (num_features * kNumOtherAgentsObserved + 18 + num_angle_lidar);
        ...
        obs_arr[kThisAgentIdxOffset + 4] = 20.0; // goal distance
        obs_arr[kThisAgentIdxOffset + 5] = 2.0;  // goal z
        obs_arr[kThisAgentIdxOffset + 6] = 0.0;  // goal dir
        obs_arr[kThisAgentIdxOffset + 7] = 1.0;  // goal speed
        obs_arr[kThisAgentIdxOffset + 8] = 0.0;  // close front pitch
        obs_arr[kThisAgentIdxOffset + 9] = 0.0;  // close back pitch
        obs_arr[kThisAgentIdxOffset + 10] = 0.0;  // close left pitch
        obs_arr[kThisAgentIdxOffset + 11] = 0.0;  // close right pitch
        obs_arr[kThisAgentIdxOffset + 12] = 0.0;  // time
        obs_arr[kThisAgentIdxOffset + 13] = 0.0;  // in gmae
        obs_arr[kThisAgentIdxOffset + 14] = 0.0;  // front pitch
        obs_arr[kThisAgentIdxOffset + 15] = 0.0;  // back pitch
        obs_arr[kThisAgentIdxOffset + 16] = 0.0;  // left pitch
        obs_arr[kThisAgentIdxOffset + 17] = 0.0;  // right pitch
        //printf("5");
        for (int idx = 0; idx < kNumOtherAgentsObserved; idx++) {
          obs_arr[kThisAgentIdxOffset + 18 + 0 * kNumOtherAgentsObserved + idx] = 20.0;  // other LOC_X
          obs_arr[kThisAgentIdxOffset + 18 + 1 * kNumOtherAgentsObserved + idx] = 20.0;  // other LOC_Y
          obs_arr[kThisAgentIdxOffset + 18 + 2 * kNumOtherAgentsObserved + idx] = 0.0;  // other SP_X
          obs_arr[kThisAgentIdxOffset + 18 + 3 * kNumOtherAgentsObserved + idx] = 0.0;  // other SP_Y
          obs_arr[kThisAgentIdxOffset + 18 + 4 * kNumOtherAgentsObserved + idx] = 0.0;  // other DIR
          obs_arr[kThisAgentIdxOffset + 18 + 5 * kNumOtherAgentsObserved + idx] = 0.0;  // other agent size
        }

        for (int angle = 0; angle < num_angle_lidar; angle++){
          obs_arr[kThisAgentIdxOffset + 18 + num_features * kNumOtherAgentsObserved + angle] = 0.0;  // local map infomation
        }
        ...
        if (still_in_the_game_arr[kThisAgentArrayIdx]) {
          ...
          for (int idx = 0; idx < min(num_valid_other_agents, kNumOtherAgentsObserved); idx++) {
            ...
            float distace_ = pow(pow(obs_spot_x_, 2) + pow(obs_spot_y_, 2), 0.5);
            obs_arr[kThisAgentIdxOffset + 18 + 0 * kNumOtherAgentsObserved + idx] = static_cast<float>(obs_spot_x_);
            obs_arr[kThisAgentIdxOffset + 18 + 1 * kNumOtherAgentsObserved + idx] = static_cast<float>(obs_spot_y_);
            obs_arr[kThisAgentIdxOffset + 18 + 2 * kNumOtherAgentsObserved + idx] = static_cast<float>(distace_);
            obs_arr[kThisAgentIdxOffset + 18 + 3 * kNumOtherAgentsObserved + idx] = static_cast<float>(speed_x_arr[kOtherAgentArrayIdx]) / (kMaxSpeed_x + kEpsilon);
            obs_arr[kThisAgentIdxOffset + 18 + 4 * kNumOtherAgentsObserved + idx] = static_cast<float>(agent_speed_y);
            obs_arr[kThisAgentIdxOffset + 18 + 5 * kNumOtherAgentsObserved + idx] = static_cast<float>(dif_turn) / (kTwoPi);
            obs_arr[kThisAgentIdxOffset + 18 + 6 * kNumOtherAgentsObserved + idx] = static_cast<float>(agent_sizes);
          }
          ...
          if (agent_types_arr[kThisAgentId] == 0) //goal obs
          {
            ...
            obs_arr[kThisAgentIdxOffset + 1] = self_row;  // this all for same.
          ...
          else
          {
            ...
            float distace_ = pow(pow(goal_spot_x_, 2) + pow(goal_spot_y_, 2), 0.5);
            obs_arr[kThisAgentIdxOffset + 4] = distace_;
            obs_arr[kThisAgentIdxOffset + 5] = goal_z_;
            obs_arr[kThisAgentIdxOffset + 6] = goal_dir_ / (kTwoPi);
            obs_arr[kThisAgentIdxOffset + 7] = goal_speed_;
   
          }
          obs_arr[kThisAgentIdxOffset + 8] = static_cast<float>(close_front_z);
          obs_arr[kThisAgentIdxOffset + 9] = static_cast<float>(close_back_z);
          obs_arr[kThisAgentIdxOffset + 10] = static_cast<float>(close_left_z);
          obs_arr[kThisAgentIdxOffset + 11] = static_cast<float>(close_right_z);
          obs_arr[kThisAgentIdxOffset + 12] = static_cast<float>(env_timestep_arr[kEnvId]) / kEpisodeLength;
          obs_arr[kThisAgentIdxOffset + 13] = static_cast<float>(1);
          obs_arr[kThisAgentIdxOffset + 14] = static_cast<float>(front_z);
          obs_arr[kThisAgentIdxOffset + 15] = static_cast<float>(back_z);
          obs_arr[kThisAgentIdxOffset + 16] = static_cast<float>(left_z);
          obs_arr[kThisAgentIdxOffset + 17] = static_cast<float>(right_z);
          ...
          for (int i = 0; i < num_angle_lidar; i++)
          {
            ...
            {
              obs_arr[kThisAgentIdxOffset + 18 + num_features * kNumOtherAgentsObserved + i] = nomal_angle_distance - smallest_distance;
            }
            else
            {
              obs_arr[kThisAgentIdxOffset + 18 + num_features * kNumOtherAgentsObserved + i] = precise_angle_distance;
            }
          }
        }
    }
  ...
  __global__ void CudaSpot_3dDoubleStep(
    int * done_arr,
    int * env_timestep_arr,
    int * num_spot,
    int * num_hit_obstacle,
    ...
    if (kThisAgentId < kNumAgents) {
      ...
      if (speed_x_arr[kThisAgentArrayIdx] <= kMinSpeed_x * skill_levels_arr[kThisAgentId])  {
          acceleration_x_arr[kThisAgentArrayIdx] = 0;
      }
      ...
      if (speed_y_arr[kThisAgentArrayIdx] <= kMinSpeed_y * skill_levels_arr[kThisAgentId]){
          acceleration_y_arr[kThisAgentArrayIdx] = 0;
      }
