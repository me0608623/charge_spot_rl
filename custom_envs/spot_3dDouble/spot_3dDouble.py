...
class Spot_3d(CUDAEnvironmentContext, Map):
    ...
    def __init__(
        ...
        self.skill_levels = [self.float_dtype(goal_speed_rate_with_spot) if self.agent_type[agent_id] == 0 else self.float_dtype(1.0) if self.agent_type[agent_id] == 1 else self.float_dtype(obstacle_speed_rate_with_spot) * random.random() for agent_id in range(self.num_agents)]
    ...
    def generate_observation(self):
        ...
        time = np.array([1 - float(self.timestep) / self.episode_length])
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.still_in_the_game[agent_id]:
                ...
                distances_from_origin = np.sqrt(diff_x ** 2 + diff_y ** 2)
                ...
                agent_view_global_obs_[2,nearest_neighbor_ids] = distances_from_origin
                agent_view_global_obs_[5,nearest_neighbor_ids] = normalized_global_obs[5, nearest_neighbor_ids] - normalized_global_obs[4, agent_id]
                ...
                obs_2 = np.array([0.0])
                obs_3 = np.array([0.0])
                obs_4 = np.array([0.0]) # goal_distance
                obs_5 = np.array([0.0])
                obs_6 = np.array([0.0])
                obs_7 = np.array([0.0])
                obs_8 = np.array([close_front_agent_z - agent_z])
                obs_9 = np.array([close_back_agent_z - agent_z])
                obs_10 = np.array([close_left_agent_z - agent_z])
                obs_11 = np.array([close_right_agent_z - agent_z])
                ...
                if agent_types[agent_id] != 0 : # spot
                    ...
                    for k in nearest_goal:
                        ...
                        distace_ = pow(pow(diff_goal_x_, 2) + pow(diff_goal_y_, 2), 0.5)
                        
                        obs_2 = np.array([agent_view_x]) # goal x
                        obs_3 = np.array([agent_view_y]) # goal y
                        obs_4 = np.array([distace_]) # goal distance
                        obs_5 = np.array([goal_map_z - agent_map_z]) # goal z
                        obs_6 = np.array([diff_dir]) / (2 * np.pi) # goal dir
                        obs_7 = np.array([self.global_state[_SP_X][self.timestep][k] / (self.spot_max_speed_x + self.eps)]) # goal speed
