  ...
  __device__ void CudaTagContinuousGenerateObservation(
    ...
    if (kThisAgentId < kNumAgents) {
        ...
        for (int idx = 0; idx < kNumOtherAgentsObserved; idx++) {
          ...
          obs_arr[kThisAgentIdxOffset + 17 + 2 * kNumOtherAgentsObserved + idx] = 0.0;  // other SP_X
          obs_arr[kThisAgentIdxOffset + 17 + 3 * kNumOtherAgentsObserved + idx] = 0.0;  // other SP_Y
          obs_arr[kThisAgentIdxOffset + 17 + 4 * kNumOtherAgentsObserved + idx] = 0.0;  // other DIR
      ...
      for (int idx = 0; idx < min(num_agent_state_obs, kNumOtherAgentsObserved); idx++) {
        ...
        obs_arr[kThisAgentIdxOffset + 17 + 2 * kNumOtherAgentsObserved + idx] = static_cast<float>(0); // still in
        obs_arr[kThisAgentIdxOffset + 17 + 3 * kNumOtherAgentsObserved + idx] = static_cast<float>(0);
