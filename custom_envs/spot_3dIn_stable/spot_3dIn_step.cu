...
extern "C" {
  ...
  __device__ void CudaTagContinuousGenerateObservation(
    ...
    if (kThisAgentId < kNumAgents) {
      ...
      for (int idx = 0; idx < min(num_valid_other_agents, kNumOtherAgentsObserved); idx++) {
        ...
        obs_arr[kThisAgentIdxOffset + 17 + 2 * kNumOtherAgentsObserved + idx] = static_cast<float>(speed_x_arr[kOtherAgentArrayIdx]) / (kMaxSpeed_x + kEpsilon);
        obs_arr[kThisAgentIdxOffset + 17 + 3 * kNumOtherAgentsObserved + idx] = static_cast<float>(agent_speed_y);
        obs_arr[kThisAgentIdxOffset + 17 + 4 * kNumOtherAgentsObserved + idx] = static_cast<float>(dif_turn) / (kTwoPi);
      ...
      if (agent_types_arr[kThisAgentId] == 0) //goal obs
      {
        ...
        front_agent_grid_x = min(max(int((loc_x_arr[kThisAgentArrayIdx] + cos_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        front_agent_grid_y = min(max(int((loc_y_arr[kThisAgentArrayIdx] + sin_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        back_agent_grid_x = min(max(int((loc_x_arr[kThisAgentArrayIdx] - cos_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        back_agent_grid_y = min(max(int((loc_y_arr[kThisAgentArrayIdx] - sin_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        left_agent_grid_x = min(max(int((loc_x_arr[kThisAgentArrayIdx] - cos_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        left_agent_grid_y = min(max(int((loc_y_arr[kThisAgentArrayIdx] + sin_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        right_agent_grid_x = min(max(int((loc_x_arr[kThisAgentArrayIdx] + cos_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        right_agent_grid_y = min(max(int((loc_y_arr[kThisAgentArrayIdx] - sin_dir * four_sign_distance * 0.15) / kGridDensity + 0.5), 0), int(grid_length_max_y - 1));
        ...
        obs_arr[kThisAgentIdxOffset + 11] = static_cast<float>(front_z);
        obs_arr[kThisAgentIdxOffset + 12] = static_cast<float>(back_z);
        obs_arr[kThisAgentIdxOffset + 13] = static_cast<float>(left_z);
        obs_arr[kThisAgentIdxOffset + 14] = static_cast<float>(right_z);
      }
      else
      {
  ...
  __global__ void CudaSpot_3dInStep(
    ...
    if (env_timestep_arr[kEnvId] <= 1 && agent_types_arr[kThisAgentId] != 1)
    {
