...
def generate_tag_env_rollout_image(
    ...
    norm = Normalize(vmin=-1 * env.grid_length_height * 2, vmax=env.grid_length_height * 1.5)  # 標準化 z 值以進行顏色映射
    ...
    # 繪製初始數據
    point = [None for _ in range(env.num_agents)]  # 初始化智能體的繪圖點
    ...
    for idx in range(env.num_agents):  # 遍歷每個智能體
        Markersize = (env.agent_size[idx] * env.agent_size[idx] + 0.02) * 5 * 20  # 計算 Marker 大小
        ...
        if idx in env.obstacles:  # 如果是障礙物
            ...
            for t in range(1, end_index + 1, 3):
                ...
                ax.plot3D(
                    ...
                    color=to_rgba(obstacle_color, alpha=0.35 * t / end_index + 0.1),  # 更加明顯的 alpha 差異
                    marker="o",
            ...
            for t in range(1, end_index + 1, 3):
                ...
                ax.plot3D(
                    ...
                    [loc_y],
                    [loc_z],
                    color=to_rgba(spot_color, alpha=0.35 * t / end_index + 0.2), # 調整 alpha 值，拉開差距
                    marker="o",
                    markersize=Markersize,
                ...
                ax.plot3D(
                    ...
                    [loc_z],
                    color=to_rgba(spot_direction_color, alpha=0.35 * t / end_index + 0.2),  # 調整 alpha 值，拉開差距
                    marker="o",
            ...
            s_ag_z = grid_map_height[min(max_j - 1, int(s_ag_y / grid_density_)), min(max_j - 1, int(s_ag_x / grid_density_))]
            ...
            point[idx] = ax.plot3D(
                ...
                grid_map_height[min(max_j - 1, int(episode_states["loc_y"][end_index - 1, idx] / grid_density_)),
                                min(max_j - 1, int(episode_states["loc_x"][end_index - 1, idx] / grid_density_))],
                color=goal_color,
                marker="o",
                markersize=Markersize,
                zorder=7  # 設置goal的zorder為7，确保其在最上面
            )[0]

            g_ag_x = episode_states["loc_x"][end_index - 1, idx]
            g_ag_y = episode_states["loc_y"][end_index - 1, idx]
            g_ag_z = grid_map_height[min(max_j - 1, int(g_ag_y / grid_density_)), min(max_j - 1, int(g_ag_x / grid_density_))]

    # Place the goal_dis calculation here, outside the loop
    goal_dis = ((abs(s_ag_x - g_ag_x)**2) + (abs(s_ag_y - g_ag_y)**2) + (abs(s_ag_z - g_ag_z)**2)) ** 0.5 
    ...
    plt.savefig(f"spot_image/{time_idx}_state_{state}_surcess_{goal_deid}_distance_{str(goal_dis)}_timestep_{end_index}_phase_{PHASE_}.png")
    plt.close()  # 关闭图表
