...
def generate_tag_env_rollout_animation(
    ...
    revese_env = 1
    if hasattr(env, 'need_reverse_env'):
        if env.need_reverse_env > 1:
            revese_env = -1
            print("reverse_env")
    if car_mode:  revese_map_z = 0.5 * env.map_limit * revese_env  # 獲取z軸的值
    else:  revese_map_z = env.map_z * revese_env  # 獲取z軸的值
    
    norm = Normalize(vmin= -1 * env.grid_length_height * 2, vmax=env.grid_length_height * 1.5)
    colors = Greys(1 - norm(revese_map_z))

    # 繪製表面並應用顏色
    ax.plot_surface(env.map_x, env.map_y,revese_map_z, facecolors=colors, rstride=1, cstride=1, alpha=0.2)
    ...
    for idx in range(env.num_agents):
        ...
        if idx in env.spots:
            ...
            point_ass[num_spot_reg].set_zorder(8)
            num_spot_reg += 1
