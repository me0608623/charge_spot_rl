...
def _create_and_push_data_placeholders_helper(
    ...
    # push network feture to the device (module network)
    num_agents = len(agent_ids)
    if hasattr(env_wrapper.env, 'network_feture'):
        network_feature_value = env_wrapper.env.network_feture
        network_feature_placeholder = np.zeros(
            (num_envs, num_agents),
            dtype=np.float32,
        )
        num_network_feature = len(network_feature_value[0])

        tensor_feed.add_data(
            name=_PREPROCESS + suffix,
            data=np.zeros(
                network_feature_placeholder.shape + (num_network_feature,),
                dtype=np.float32,
            ),
        )
