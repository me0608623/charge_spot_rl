...
class A2CE:
    ...
    def __init__(
        ...
        if action2_entropy_coeff is not None:  self.action2_entropy_coeff_schedule = ParamScheduler(action2_entropy_coeff)
        else:  self.action2_entropy_coeff_schedule = ParamScheduler(entropy_coeff)
        if action3_entropy_coeff is not None:  self.action3_entropy_coeff_schedule = ParamScheduler(action3_entropy_coeff)
        else:  self.action3_entropy_coeff_schedule = ParamScheduler(entropy_coeff)
        self.update_Critic_network = 0
    ...
    def compute_loss_and_metrics(
        ...
        log_prob = 0.0
        epsilon = 1e-8
        mean_entropy = [0.0, 0.0, 0.0]
        mean_result = [0.0, 0.0, 0.0]
        for idx in range(actions_batch.shape[-1]):
            m = Categorical(action_probabilities_batch[idx])
            mean_entropy[idx] += m.entropy().mean()
            logp = torch.log(m.probs + epsilon)
            
            result = 0
            n = m.probs.size(-1)
            p = torch.zeros(n)
            for i in range(n):
                p[i] = (m.probs[..., i] * logp[..., i]).mean()
            for i in range(n // 2):
                result += abs(p[i] - p[n -1 -i])
            mean_result[idx] = 2 * result / (n // 2) 
        ...
        action2_entropy_coeff_t = self.action2_entropy_coeff_schedule.get_param_value(timestep)
        action3_entropy_coeff_t = self.action3_entropy_coeff_schedule.get_param_value(timestep)

        
        max_policy_loss = abs(policy_loss.item())
        if max_policy_loss > 8:
            policy_loss = policy_loss * 8.0 / max_policy_loss
        if abs(vf_loss_coeff_t * vf_loss.item()) > 30:
            max_30 = abs(max(min(30.0, max_policy_loss), 10.0) / vf_loss.item())
            vf_loss = vf_loss * max_30
        self.loss_coefficient_entropy = math.log10(abs(policy_loss.item())) - math.log10(vf_loss_coeff_t * abs(vf_loss.item()))
        if self.update_Critic_network != 1:
            loss = policy_loss + vf_loss_coeff_t * vf_loss - entropy_coeff_t * mean_entropy[0] - action2_entropy_coeff_t * mean_entropy[1] - action3_entropy_coeff_t * mean_entropy[2]
            if loss > 0:  loss += action2_entropy_coeff_t * mean_result[1] + action3_entropy_coeff_t * mean_result[2]
            else:  loss -= action2_entropy_coeff_t * mean_result[1] + action3_entropy_coeff_t * mean_result[2]
        else:
            loss = 0 * vf_loss
        ...
        if perform_logging:
            metrics = {
                ...
                "Mean entropy": mean_entropy[0].item() + mean_entropy[1].item() + mean_entropy[2].item(),
