...
class Spot_Env():
    ...
    def replace_old_file(self, wandb_path):
        ...
        python_code_path = now_python_path # self.search_files_with_keywords(wandb_path, keywords = ['cuda_code'])
        shutil.copy2(now_python_path, reg_python_path)
        #shutil.copy2(python_code_path, now_python_path)
    ...
    def test(self,time_id, artifact_version, Start_Testing_type = 1, End_test_type = 2 ,test_num_time = 5, model_connect = None):
        ...
        for Testing_type_ in range(5):
            ...
            avoid_idx = 215
            avoid_seed = [3, 5, 6, 7, 9, 11, 12, 13, 14, 16, 19]  #9, 25, 28, 33
            run_time = [10, 15, 60]
            ...
            for time in range(test_num_time):
                if 2 + time + avoid_idx in avoid_seed:
                    ...
                    if 2 + time + avoid_idx in avoid_seed:
                        ...
                        if 2 + time + avoid_idx in avoid_seed:
                            avoid_idx += 1
                self.phase_config['seed'] = run_time[time] #time + 680 #
